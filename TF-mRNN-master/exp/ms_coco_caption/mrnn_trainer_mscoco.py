...
flags.DEFINE_string(
    ...
    "anno_list_mscoco_trainModelVal_m_RNN.pickle",
...
flags.DEFINE_string("vocab_path", 
    "./cache/dctionary/mscoco_mc0_vocab", 
...
def main(unused_args):
  ...
  with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(
    ...
    print(config.vf_size)
    data_provider = mRNNCocoBucketDataProvider(FLAGS.anno_files_path.split(':'),
        FLAGS.vocab_path, config.vocab_size, FLAGS.vf_dir, config.vf_size,config.seq_len)
